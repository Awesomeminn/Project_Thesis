...
url = "http://ecos.bok.or.kr/api/StatisticSearch/33RX7OBHFFA28P4I07F3/json/kr/1/1000/901Y098/M/201301/202412/I74I/I74C/"
response = requests.get(url)
data = response.json()
rdata = data['StatisticSearch']["row"]
df = pd.DataFrame(rdata)
df
## 1.3 필요한 칼럼만 가져와서 supermarket이라는 새로운 데이터프레임 만들기
columns_to_keep = ['TIME', 'DATA_VALUE']
supermarket = df[columns_to_keep].copy()
supermarket = supermarket.rename(columns={'TIME': 'Date', 'DATA_VALUE': 'Value'})
supermarket['Date'] = supermarket['Date'].str[:4] + '-' + supermarket['Date'].str[4:] ## 연,월 구분
supermarket
## 1.4 자료형태 변경
supermarket['Date'] = pd.to_datetime(supermarket['Date'])
supermarket['Value'] = supermarket['Value'].astype(float)
supermarket.info()
#======================================================================================================#
# 2. 수준변수에 대해서 ADF 검정
from statsmodels.tsa.stattools import adfuller
series = supermarket['Value']
## 2.1 aic 기준 / 상수항만 포함
...
import numpy as np
supermarket['log_diff'] = np.log(supermarket['Value']).diff()
diff_series = supermarket['log_diff'].dropna()
## 3.1 aic 기준 / 상수항만 포함
...
from 대형마트판매액지수 import large_supermarket
## 4.2 데이터 합치기
log_diff_dict = {
    'News': News['log_diff'],
    'curr_econ_CSI': curr_econ_CSI['log_diff'],
    'large_supermarket': large_supermarket['log_diff'],
    'supermarket':supermarket['log_diff']
}
dates = News['Date']
...
merge
#======================================================================================================#
merge.to_csv('merge2.csv', index=True)
